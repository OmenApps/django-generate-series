...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-generate-series runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-generate-series.md")
    c.run("rm -f docs/index.md")
    c.run("sphinx-apidoc -o docs/ django_generate_series")
...
def lint(c):
    ...
    """
    c.run("flake8 django-generate-series tests")
...
def release(c, bumpsize=""):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_generate_series.__version__))
    c.run("git push --tags")
